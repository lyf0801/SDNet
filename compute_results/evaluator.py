...
class Eval_thread():
    ...
    def run(self):
        ...
        self.LOG(
            '{} ({}): {:.4f} mae || {:.4f} max-fm || {:.4f} S-measure || {:.4f} E-measure.\n'
            .format( self.dataset, self.method, mae, max_f, s, mean_e))
        
        return '[cost:{:.4f}s] {} ({}): {:.4f} mae || {:.4f} max-fm || {:.4f} S-measure || {:.4f} E-measure.'.format(
            time.time() - start_time, self.dataset, self.method, mae, max_f, s, mean_e), max_f
